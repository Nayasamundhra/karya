...
4. Render's own Git auto-deploy and Vercel's own Git auto-deploy are both
   turned off (`render.yaml`'s `autoDeploy: false`; Vercel project setting).
   The only path to production is the pipeline above, so a failing test
   run can never reach real users.
